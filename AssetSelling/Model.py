...
class AssetSellingModel:
    ...
    def __init__(
        ...
        print(f"========= {exog_params} ========")
        biasdf = pd.DataFrame(
            data=exog_params["bias_df"]["rows"],
            columns=exog_params["bias_df"]["columns"],
            index=exog_params["bias_df"]["columns"],
        )
        # biasdf = exog_params["bias_df"]
    ...
    def exog_info_fn(self):
        ...
        if coin < biasprob["Up"]:
            new_bias = "Up"
            bias = exog_params["up_step"]
        elif coin >= biasprob["Up"] and coin < biasprob["Neutral"]:
            new_bias = "Neutral"
            bias = 0
        else:
            new_bias = "Down"
            bias = exog_params["down_step"]
