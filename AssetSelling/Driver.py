...
from Model import AssetSellingModel
...
if __name__ == "__main__":
    ...
    t = 0
    prev_price = config["init_state"]["price"]
    ...
    for policy in config["policies"]:
        name = policy["name"]
        policy_info[name] = policy["params"]
        if name == "track":
            param_list.append(tuple(policy["params"] + [prev_price]))
        else:
            param_list.append(tuple(policy["params"]))

    policy_selected = config["simulation"]["policy"]
    if not policy_selected == "full_grid":
        print(
            "Selected policy {}, time horizon {}, initial price {} and number of iterations {}".format(
                ...
                config["init_state"]["price"],
                config["simulation"]["iterations"],
            )
        )
        contribution_iterations = [
            P.run_policy(param_list, policy_info, policy_selected, t)
            for ite in list(range(config["simulation"]["iterations"]))
        ]
